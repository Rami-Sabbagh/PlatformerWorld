...
function Loader:init(Holders)
	...
	for i=1,numThreads do 
		local thread = love.thread.newThread("Engine/LoaderThread.lua");
		thread:start();
		channel:supply(i);
...
function Loader:load(FilePath,Name,FontSize,SpecialRequest,Prefix)
	...
	local exceptions = {"db"} ---to skip files, (used to be to be to skip json files)
...
local LoadFonts = function()
	...
	for i=1,10 do 
		if(#fontsToLoad > 0)then
			...
			fontHolder[ob.name] = love.graphics.newFont(ob.filePath,ob.fontsize);
